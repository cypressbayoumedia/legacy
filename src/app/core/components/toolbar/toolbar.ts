...
@Component({
  ...
  template: `
    <div class="toolbar" role="toolbar" aria-label="Main Navigation">
      <div class="logo">
          <img src="assets/logo.svg" alt="Legacy" style="height: 48px; width: auto;">
      </div>
  ...
  styles: [`
    ...
    .toolbar {
      ...
      padding: 1rem 2rem; background: #000; border-bottom: 1px solid #222;
    }
    .logo { font-weight: 700; font-size: 1.25rem; }
    .nav-items { display: flex; gap: 1rem; align-items: center; }
    a, button.sign-out-btn {
      text-decoration: none; color: #888; padding: 0.5rem 1rem; border-radius: 4px;
      transition: all 0.2s; outline: none; border: none; background: transparent; font-size: 1rem; font-family: inherit; cursor: pointer;
    }
    a:hover, button.sign-out-btn:hover { background: #222; color: #fff; }
    a.active { font-weight: 600; color: #fff; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.1); }
